#!/usr/bin/env bash
# Run a CVE scan on the npm catalog using osv-scanner (via Docker).
#
# Prerequisites:
#   - Docker
#   - public/data/javascript/catalog.json exists (run fetch_npm_data.py first)
#
# Writes: public/data/javascript/audit.json
set -euo pipefail

DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DIR"

CATALOG="$DIR/../public/data/javascript/catalog.json"
RESULTS="$DIR/results/npm"
LOCKFILE="$RESULTS/package-lock.json"
OSV_RAW="$RESULTS/osv_raw.json"
OSV_IMAGE="ghcr.io/google/osv-scanner:latest"

if [ ! -f "$CATALOG" ]; then
  echo "ERROR: $CATALOG not found. Run fetch_npm_data.py first." >&2; exit 1
fi
...
# osv-scanner exits 1 when vulns found — expected, not a failure.
# Mount the results dir so the container can read the lockfile.
docker run --rm \
  -v "$RESULTS:/scan:ro" \
  "$OSV_IMAGE" \
...
python3 normalize_npm_audit.py
